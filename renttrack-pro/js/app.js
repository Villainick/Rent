...
let state = {
  units: {},
  rent: {},       // rent[unitId][YYYY-MM] = {status, amountPaid, payDate, note}
  electricity: {}, // electricity[unitId][YYYY-MM] = {oldReading, newReading, rate, bill, status}
  expenses: {},   // expenses[id] = {category, amount, date, note}
  maintenance: {}, // maintenance[id] = {unit, desc, status, date, schedule}
  settings: { elecRate: 8, pin: '1234', theme: 'dark' }
};

let pinBuffer = '';
let pinMode = 'verify'; // 'verify' or 'set'
...
function initPinScreen() {
  const hasPin = state.settings && state.settings.pin;
  if (!hasPin) {
    pinMode = 'set';
    document.getElementById('pinLabel').textContent = 'Set a 4-digit PIN';
  }
}
...
async function changePin() {
  ...
  if (current !== state.settings.pin) { showToast('❌ Current PIN is wrong'); return; }
  if (newP.length !== 4) { showToast('❌ PIN must be 4 digits'); return; }
  if (newP !== confirm) { showToast('❌ PINs do not match'); return; }
  state.settings.pin = newP;
  await saveAll();
